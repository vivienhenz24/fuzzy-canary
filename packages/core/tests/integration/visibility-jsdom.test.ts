/**
 * jsdom-based E2E tests for canary placement
 *
 * These tests provide the same coverage as the Playwright E2E tests but use jsdom instead.
 * Benefits over Playwright:
 * - Faster execution (no browser startup)
 * - Lighter weight (no browser binaries required)
 * - Easier to debug with standard Node.js debugging
 * - Better for AI-assisted development (no visual rendering issues)
 * - Runs in the same environment as other unit/integration tests
 *
 * The tests verify:
 * - Canary injection into DOM (off-screen nodes, comments)
 * - Proper styling to hide from users but remain in DOM for scrapers
 * - Bot detection (search engines vs. malicious scrapers)
 * - Content quality (natural language, no keyword stuffing)
 * - Consistent canary text across different DOM locations
 */
...
describe('E2E - best-practice canary placement (jsdom)', () => {
  ...
  const loadTestPage = (userAgent?: string) => {
    let html = readFileSync(join(__dirname, '../fixtures/test-page.html'), 'utf-8')
    const runtimeScript = readFileSync(join(__dirname, '../fixtures/runtime.min.js'), 'utf-8')

    // Remove the external script tag reference
    ...
    // Inject the runtime script directly
    ...
    if (lastScript && lastScript.parentNode) {
      lastScript.parentNode.insertBefore(scriptEl, lastScript)
    } else {
